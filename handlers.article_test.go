...
func TestArticleAuthenticated(t *testing.T) {
	...
	// parse and process HTML pages
	p, err := ioutil.ReadAll(w.Body)
	if err != nil || !strings.Contains(string(p), "<title>Article 1</title>") {
		t.Fail()
	}
...
func TestArticleCreationPageAuthenticated(t *testing.T) {
	...
	// parse and process HTML pages
	p, err := ioutil.ReadAll(w.Body)
	if err != nil || !strings.Contains(string(p), "<title>Create New Article</title>") {
		t.Fail()
	}
...
func TestArticleCreationAuthenticated(t *testing.T) {
	...
	// parse and process HTML pages
	p, err := ioutil.ReadAll(w.Body)
	if err != nil || !strings.Contains(string(p), "<title>Submission Successful</title>") {
		t.Fail()
	}
